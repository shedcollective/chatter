...
@section(Config::get('chatter.yields.head'))
    @if(Config::get('chatter.sidebar_in_discussion_view'))
        <link href="{{ url('/vendor/shedcollective/chatter/assets/vendor/spectrum/spectrum.css') }}" rel="stylesheet">
    @endif
    <link href="{{ url('/vendor/shedcollective/chatter/assets/css/chatter.css') }}" rel="stylesheet">
    @if($chatter_editor == 'simplemde')
        <link href="{{ url('/vendor/shedcollective/chatter/assets/css/simplemde.min.css') }}" rel="stylesheet">
    @elseif($chatter_editor == 'trumbowyg')
        <link href="{{ url('/vendor/shedcollective/chatter/assets/vendor/trumbowyg/ui/trumbowyg.css') }}" rel="stylesheet">
        <style>
                ...
                <div class="col-md-3 left-column">
                    ...
                    <div class="chatter_sidebar">
                        ...
                        <ul class="nav nav-pills nav-stacked">
                            <?php $categories = ShedCollective\Chatter\Models\Models::category()->all(); ?>
	                		...
	                		<li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
		                		<span class="chatter_posts">
			                		...
			                		<div class="chatter_avatar">
					        			...
					        			@else
					        				<span class="chatter_avatar_circle" style="background-color:#<?= \ShedCollective\Chatter\Helpers\ChatterHelper::stringToColorCode($post->user->{Config::get('chatter.user.database_field_with_user_name')}) ?>">
					        		...
					        		<div class="chatter_middle">
					        			<span class="chatter_middle_details"><a href="{{ \ShedCollective\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
					        			<div class="chatter_body">

					        				@if($post->markdown)
					        					<pre class="chatter_body_md">{{ $post->body }}</pre>
					        					<?= \ShedCollective\Chatter\Helpers\ChatterHelper::demoteHtmlHeaderTags( GrahamCampbell\Markdown\Facades\Markdown::convertToHtml( $post->body ) ); ?>
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="chatter_avatar">
		        			...
		        			@else
		        				<span class="chatter_avatar_circle" style="background-color:#<?= \ShedCollective\Chatter\Helpers\ChatterHelper::stringToColorCode(Auth::user()->{Config::get('chatter.user.database_field_with_user_name')}) ?>">
						...
						<div id="discussion_response_email">
							...
							@if(Config::get('chatter.email.enabled'))
								<div id="notify_email">
									<img src="{{ url('/vendor/shedcollective/chatter/assets/images/email.gif') }}" class="chatter_email_loader">
...
@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
    ...
    <script src="{{ url('/vendor/shedcollective/chatter/assets/vendor/tinymce/tinymce.min.js') }}"></script>
    <script src="{{ url('/vendor/shedcollective/chatter/assets/js/tinymce.js') }}"></script>
    <script>
...
@elseif($chatter_editor == 'simplemde')
    ...
    <script src="{{ url('/vendor/shedcollective/chatter/assets/js/simplemde.min.js') }}"></script>
    <script src="{{ url('/vendor/shedcollective/chatter/assets/js/chatter_simplemde.js') }}"></script>
@elseif($chatter_editor == 'trumbowyg')
	<script>var chatter_editor = 'trumbowyg';</script>
    <script src="{{ url('/vendor/shedcollective/chatter/assets/vendor/trumbowyg/trumbowyg.min.js') }}"></script>
    <script src="{{ url('/vendor/shedcollective/chatter/assets/vendor/trumbowyg/plugins/preformatted/trumbowyg.preformatted.min.js') }}"></script>
    <script src="{{ url('/vendor/shedcollective/chatter/assets/js/trumbowyg.js') }}"></script>
@endif

@if(Config::get('chatter.sidebar_in_discussion_view'))
    <script src="/vendor/shedcollective/chatter/assets/vendor/spectrum/spectrum.js"></script>
@endif
...
<script src="{{ url('/vendor/shedcollective/chatter/assets/js/chatter.js') }}"></script>
